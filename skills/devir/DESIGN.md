...
| **L2 — Lokal not (opt-in commit)** | `<ana-worktree>/.claude/docs/devir-notes/<id>.md` | **ana checkout'ta** (paylaşılan + kalıcı), **varsayılan git-ignored**, proje-bazlı | **Durable** (ana worktree prune olmaz → not kaybolmaz; linked worktree'de bile buraya yazılır) **+ cross-machine** yalnız **opt-in commit** (`git add -f`) ile |
...
- **Non-destructive:** resume hiçbir şeyi **silmez**; `consumed` sadece frontmatter flip'i (geri alınabilir). Silme/arşivleme **her zaman manuel**. Arşiv: `devir-notes/archive/` (scan oraya bakmaz). Manuel arşiv yolu = **`/devir-archive`** (spent=consumed/superseded + bayat=open/draft >`ARCHIVE_ADVISORY_DAYS`g adaylarını onayla → `mv`, asla `rm`, geri-dönülebilir); SessionStart count-only `🗄️ arşiv adayı` banner'ı bunu önerir — **otomatik taşıyıcı yok** (auto-archive belgelenmiş invariant ihlali + flock'suz yarış).
...
| **2.3** | 2026-06-16 | **L2 not dayanıklılık fix** (§2). Kök neden: `notes_dir` `git rev-parse --show-toplevel` ile çözülüyordu → linked worktree İÇİNDE worktree'nin kendi (ephemeral) köküne yazıyordu; gitignored olduğundan worktree prune'unda not **sessizce kayboluyordu** (+ her worktree ayrı dizin görüyordu → cross-worktree görünmezlik). Fix: `main_worktree_root` (`--git-common-dir` ebeveyni) ile **ana checkout'a** çapalandı → tek paylaşılan + kalıcı dizin. `/devir-resume` Faz 1 filtresi worktree-yolu→**branch-match** birincil sinyaline gevşetildi. `repo_root` (worktree-kimliği için) değişmedi; SessionStart aynı-session compact-inject strict kalır. |
| **2.4** | 2026-06-17 | **Arşiv hijyeni — advisory banner + `/devir-archive`** (§3, B-retention'ın non-destructive hali). Paylaşılan dizin zamanla harcanmış/bayat notla şişiyordu. SessionStart startup/resume banner'ına **count-only** `🗄️ arşiv adayı` ipucu eklendi (`ARCHIVE_ADVISORY_DAYS=14`): spent=`consumed`/`superseded` + stale=`open`/`draft` mtime >14g. Hiçbir notu **gizlemez/taşımaz/inject'i kesmez** — yaş-cutoff yalnız ipucu (open/draft inject/precedence invariant'ı korunur). Yeni **`/devir-archive`** skill (manuel, `disable-model-invocation: true`): adayları kovalar → **SOR** → seçilenleri `archive/`'e **`mv`** (asla `rm`, geri-dönülebilir; taze open/draft'a dokunmaz). Reddedilen B (auto-archive) bilinçli ERTELENDİ — invariant ihlali + flock'suz yarış. |
